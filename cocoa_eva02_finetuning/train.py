"""EVA-02-Large fine-tuning on merged cocoa-pod YOLO crops — PyTorch + ROCm.

Single-phase fine-tune with:
  - WeightedRandomSampler (handles 35:1 imbalance: healthy ≫ witches_broom)
  - timm Mixup/CutMix (reduced alphas)
  - AdamW + cosine warmup
...
def train(cfg, splits, out_dir: Path):
    ...
    # ---- datasets ---- #
    train_ds = CocoaDataset(splits["train"], build_transforms(cfg, True))
    val_ds = CocoaDataset(splits["val"], build_transforms(cfg, False))
...
def main():
    ...
    ap.add_argument("--output", type=Path, default=Path("runs/eva02_l_v1"),
